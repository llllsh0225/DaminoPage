...
if (t_arr != null) {
	...
	totalPizzaPrice = (parseInt(pizzaArr[i].p_price, 10) * parseInt(pizzaArr[i].p_count, 10));
	pizza_total += totalPizzaPrice;
...
var sidePrice = 0;
	if (s_Arr != null) {
			for (var s = 0; s < s_Arr.length; s++) {
				alert("s_Arr.length : " +  s_Arr.length);
			...
			/* 	
				console.log("side_total : " + side_total);
...
function sum(){
	...
	$('#total-price').html(Number(pizza_total + side_total));
	//$('#total-price').html(Number(pizza_total));
	var total = $('#total-price').text();
	total = total.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
	$('#total-price').html(total); 
